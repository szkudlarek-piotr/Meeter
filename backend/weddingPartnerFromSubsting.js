...
async function getPeopleFromSubstring(deliveredSubstring) {
    ...
    let returnedArray = []
    const [ queryResult ] = await pool.query(`SELECT * FROM party_people WHERE CONCAT(name, ' ', surname) LIKE ? AND gender = "F" AND klika_id != 8 AND id != 30 ORDER BY surname, name;`, [substringWithPercents])
    //odrzucam klikę ósmą, bo to osoby z rodziny
